        ...
        if (s.y_range) {
            ...
            if (s.is_stacked) {
                ...
                while (++i<json[0].data.length) {
                    var j=-1, t = 0;
                    while (++j<json.length) t += json[j].data[i].value;
                    if (t > max) max = t;
                }
                y_dom = [
            ...
            if (s.is_stacked) {
                if (j > 0) {
                    bars.attr("transform", function(d, i) {
                        return "translate(" + s.x(i) + "," + (s.y(d.value) - height + s.y(json[j-1].data[i].value)) + ")";
                    });
                } else {
                    bars.attr("transform", function(d, i) { return "translate(" + s.x(i) + "," + s.y(d.value) + ")";  });
                }
            } else {
                bars.attr("transform", function(d, i) { return "translate(" + (s.x(i) + s.x_shift*j) + "," + s.y(d.value) + ")";  });
            }
