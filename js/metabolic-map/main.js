define(["vis/scaffold", "lib/d3"], function (scaffold, d3) {
    return function(options) {
        ...
        var files_to_load = [{file: o.css_path, callback: set_css },
                             ...
                             {file: o.flux_path,  callback: function(f) { set_flux(f, 0); } },
                             {file: o.flux2_path, callback: function(f) { set_flux(f, 1); } }
                            ];
        scaffold.load_files(files_to_load, function(out) {
            console.log('loaded');
            update();
        });

	// TODO deprecate the maps object
        var maps = {};

        return {update: update};

	// Definitions
        function set_css(css) { o.css = css; };
        function set_map(map_data) { o.map_data = map_data; };
        function set_flux(flux, index) {
        ...
        function update() {
            ...
            svg.append("g")
                ...
                .text(function(d) {
                    ...
                    else if (maps.has_metabolite_deviation) {
                    ...
                    else if (maps.has_metabolites) t += " (0)";
                    return t;
                })
                .attr("font-size", function(d) {
