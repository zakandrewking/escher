 ...
 */
function _convert_to_conditional_stream (status_stream, name) {
  // true if hold is pressed
  const is_not_hold_event = status_stream
          .map(x => x === 'hold')
          .not()
          .toProperty(true)
  const is_not_first_clear = status_stream
          .scan(false, function(c, x) {
            // first clear only
            return (c==false && (x=='accepted' || x=='rejected'))
          }).not()
  const is_not_first_hold = status_stream
          .scan(false, function(c, x) {
            // first clear only
            return (c==false && x=='hold')
          }).not()
  const combined = bacon.combineAsArray(this, status_stream)
  const held = combined
          .scan([], function(c, x) {
            if (name === 'allow_building_duplicate_reactions') console.log(c, x)
            if (x[1] === 'hold') {
              c.push(x[0])
              return c
            } else if (x[1] === 'accept') {
  ...
  const unheld = this.filter(is_not_hold_event)
  return unheld.merge(held)
}

/**
 * Combine the stream with another stream (bus) that is used to force a new
 * value.
 */
...
function _create_conditional_setting (name, initial_value, set_option,
  ...
  // conditionally accept changes
        .convert_to_conditional_stream(status_bus, name)
  // force updates
        .force_update_with_bus(force_update_bus)

  // get the latest
  bus.onValue(function (v) {
    if (name === 'allow_building_duplicate_reactions') console.log('bus', name, v)
    set_option(name, v)
  })

  // get the latest
  stream.onValue(function (v) {
    if (name === 'allow_building_duplicate_reactions') console.log('stream', name, v)
    set_option(name, v)
  })
